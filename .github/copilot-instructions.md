...
- **Never hardcode secrets or tokens.** API keys and client IDs must come from environment variables (`import.meta.env.VITE_*`), not source code
- Tokens received from Spotify (implicit flow) are short-lived; do not persist the raw access token beyond the current session
- `localStorage` stores serialised user objects — never include the raw token in the stored value
